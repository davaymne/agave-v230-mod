...
impl ReplayStage {
    pub fn new(
        ...
        let run_replay = move || {
            ...
            let mut last_logged_vote_slot: Slot = u64::MAX; //[BINGO]
            ...
            loop {
                ...
                for slot in newly_computed_slot_stats {
                    ...
                    let (duplicate_confirmed_forks, mostly_confirmed_slots) = Self::tower_duplicate_confirmed_forks( //[BINGO]
                        &tower,
    ...
    fn first_mostly_confirmed_bank(bank: Arc<Bank>, progress: &ProgressMap) -> Option<Arc<Bank>> {
        if progress
            .get_fork_stats(bank.slot())
            .unwrap()
            .mostly_confirmed_hash.is_some() //[BINGO]
        {
            Some(bank.clone())
    ...
    #[allow(clippy::too_many_arguments)]
    fn handle_votable_banks( // [BINGO]
        banks: &Vec<Arc<Bank>>, // [BINGO]
        ...
        // [BINGO] Stop
            ...
            let new_root = tower.record_bank_vote(bank, pop_expired); // [BINGO] Stop
        ...
        info!("voting for window: {:?}", new_slots); // [BINGO]
        Self::push_vote(
            banks.last().unwrap(), // [BINGO]
            vote_account_pubkey,
            identity_keypair,
            authorized_voter_keypairs,
            tower,
    ...
    fn mark_slots_duplicate_confirmed(
        ...
        mostly_confirmed_forks: &[(Slot, Hash)], //[BINGO]
        blockstore: &Blockstore,
        bank_forks: &RwLock<BankForks>,
        progress: &mut ProgressMap,
        duplicate_slots_tracker: &mut DuplicateSlotsTracker,
        ...
        //[BINGO] start
        for (slot, hash) in mostly_confirmed_forks.iter() {
            progress.set_mostly_confirmed_hash(*slot, *hash);
        }
        //[BINGO] end
    }
    ...
    fn tower_duplicate_confirmed_forks(
        ...
        progress: &ProgressMap,
        bank_forks: &RwLock<BankForks>,
    ) -> (Vec<(Slot, Hash)>, Vec<(Slot, Hash)>) { //[BINGO]
        let mut duplicate_confirmed_forks = vec![];
        let mut mostly_confirmed_forks = vec![]; //[BINGO]
        ...
        (duplicate_confirmed_forks, mostly_confirmed_forks) //[BINGO]
    }
...
pub(crate) mod tests {
    ...
    fn test_compute_bank_stats_confirmed() {
        ...
        // bank 1, so no slot should be confirmed.
        {
            ...
            let (confirmed_forks, _) = ReplayStage::tower_duplicate_confirmed_forks( //[BINGO]
                &tower,
    ...
    fn test_unconfirmed_duplicate_slots_and_lockouts_for_non_heaviest_fork() {
        ...
        */
        ...
        tower.record_bank_vote(&bank_forks.read().unwrap().get(5).unwrap(), true); //[BINGO]
    ...
    fn test_unconfirmed_duplicate_slots_and_lockouts() {
        ...
        */
        ...
        tower.record_bank_vote(&bank_forks.read().unwrap().get(4).unwrap(), true); //[BINGO]
    ...
    fn setup_vote_then_rollback(
        ...
        */
        ...
        tower.record_bank_vote(&bank_forks.read().unwrap().get(first_vote).unwrap(), true); //[BINGO]
    ...
    fn test_replay_stage_refresh_last_vote() {
        ...
        tower.record_bank_vote(&bank0, true); //[BINGO]
        ...
        // not landing
        tower.record_bank_vote(&bank1, true); //[BINGO]
        ReplayStage::push_vote(
            &bank1,
            &my_vote_pubkey,
            &identity_keypair,
    ...
    fn send_vote_in_new_bank(
        ...
        let my_vote_pubkey = &my_vote_keypair[0].pubkey();
        tower.record_bank_vote(&parent_bank, true); //[BINGO]
        ReplayStage::push_vote(
            &parent_bank,
            my_vote_pubkey,
            identity_keypair,
